...
Current version : 29-November-2020
...
(12) Added ability to add codon usage profiles to Domain class objects
"""
...
class Domain:
	...
	# codonUsage	     : np.array, the %MinMax profile generated based on this domain's DNA sequence
	...
	# function to add codon usage profiles to Domain objects
	def addCodonUsageProfile(self, codonUsage):
...
def alignRandomReadProfiles(pairs, domains1, domains2=None):
	...
	for pair in pairs:
		...
		if domains2 != None:
			...
			#print ('AlignRandomReadProfiles:', d1, len(domains1[d1].singleDomainProfile), d2, len(domains1[d2].singleDomainProfile))
			temp = alignSingleDomainProfiles(domains1[d1].singleDomainProfile, domains1[d2].singleDomainProfile, length)
			if temp[0] is None or temp[1] is None:
				print ('Error:', d1, len(domains1[d1].singleDomainProfile), d2, len(domains1[d2].singleDomainProfile))
				continue
		alignedProfiles[d1+':'+d2] = [ normProfile(temp[0][:,1]), normProfile(temp[1][:,1]), ]
	return alignedProfiles

# function to perform naive domain profile alignments for randomly selected profiles
def alignRandomReadProfiles_minmax(pairs, domains1, domains2=None):
	...
	# Note: identical to alignRandomReadProfiles except this version uses alignSingleDomainProfiles_minmax
	...
	for pair in pairs:
		...
		if domains2 != None:
			...
			length = min([len(domains1[d1].singleDomainProfile), len(domains1[d2].singleDomainProfile)])
			#print ('AlignRandomReadProfiles:', d1, len(domains1[d1].singleDomainProfile), d2, len(domains1[d2].singleDomainProfile))
			temp = alignSingleDomainProfiles_minmax(domains1[d1].singleDomainProfile, domains1[d2].singleDomainProfile)
			if temp[0] is None or temp[1] is None:
				print ('Error:', d1, len(domains1[d1].singleDomainProfile), d2, len(domains1[d2].singleDomainProfile))
				continue
		alignedProfiles[d1+':'+d2] = [ normProfile(temp[0][:,1]), normProfile(temp[1][:,1]), ]
...
def runComparison(alignedProfiles, domains, outdir, n, k, nComp=19, domains2=None, verbose=False):
	...
	# domains2 (optional): dictionary or None, if supplied then enables the use of a second dictionary from which to draw comparison profiles
	...
	for pair in alignedProfiles:
		compDomains = chooseUnrelatedDomains(pair.split(':')[k], domains, len(alignedProfiles[pair][0]), verbose=verbose, domains2=domains2)
		c2 = 0
...
# function that actually carries out comparisons between related and unrelated translation rate profiles
def runEcoliComparison(alignedProfiles, domains, outdir, n, k, nComp=19, domains2=None):
	...
	for pair in alignedProfiles:
		...
		# write out the profile for plotting
		if pair.split(':')[0] == 'YER069W;541-696':
			outprofile = open('mohammed_versus_pooled_k=0_30-100/'+pair.split(':')[0].split(';')[0]+'_'+pair.split(':')[0].split(';')[1]+pair.split(':')[1].split(';')[0]+'_'+pair.split(':')[1].split(';')[1]+'.txt', 'w')
			profile1 = alignedProfiles[pair][0]
			profile2 = alignedProfiles[pair][1]
			outprofile.write('#'+pair+'\n')
			for i in range (0, len(profile1)):
				outprofile.write(str(i+1)+'\t'+'%.9f' %profile1[i]+'\t'+'%.9f' %profile2[i]+'\n')
			outprofile.close()
			sys.exit()
...
# function to take in two singleDomainProfile arrays generated from minmax data and then output the aligned region
def alignSingleDomainProfiles_minmax(profile1, profile2):

        # profile1: np.array; singleDomainProfile 1
        # profile2: np.array; singleDomainProfile 2
        # length  : int; length of the paralogous domain aligned profiles; must make sure all profiles are exactly the same length

	# Return the entire length of the aligned profile, not just a subset of it to match a particular length
...
def permutationTest(file_list1, file_list2, samples=10000, alpha=0.05, logfile='permutation_tests.log'):
	...
	# this function is currently set up to work only with 20 data points in each set of files
	if len(file_list1) != 20 or len(file_list2) != 20:
		print ('File lists must contain exactly 20 files!')
		print ('file_list1 length:', len(file_list1))
		print ('file_list2 length:', len(file_list2))
		sys.exit()
	...
	outfile.write('#Rank,|T_obs|,Npass,Nsamples\n')
	...
	for rank in range (0, 20):
		...
		for i in range (0, samples):
			np.random.shuffle(all_data) # randomly shuffle the data
			randSamp1 = all_data[0:20] # get first 20 values
			randSamp2 = all_data[-20:] # get last 20 values
		...
		#pline = 'Rank'+str(rank+1)+': permutation absolute difference was greater than |T_obs| = '+'%.5f' %T_obs
		#pline += 'in '+str(greater_abs_diff_count)+' out of '+str(samples)+' random shufflings'+'\n'
		pline = str(rank+1)+','+'%.5f' %T_obs+','+str(greater_abs_diff_count)+','+str(samples)+'\n'
		outfile.write(pline)
	return

# convert string 'True' to Boolean True, return False for all other strings/values
def str2bool(string):
	if string == 'True':
		return True
	else:
...
# run comparisons of codon usage profiles - this is effectively the same as for normal profile
# comparisons between ribosome occupancy profiles. The salient difference is that we for the codon usage
# analysis we want to perform comparisons for exactly the same set of domains as for the ribosome occupancy
# analysis
def runCodonUsageComparisons(out_path, alignedDomains, domains, comparisons, map_comp_to_type):

	# out_path: str, file to which output data will be written
	# alignedDomains: dictionary; keys are of the form 'YAL017W;1090-1354:YCL024W;18-286'
	#                             values are a list of np.arrays corresponding to the array profiles
	# domains: dictionary; keys are strings of the form 'orfID;resStart-resEnd'
	#                      values are corresponding Domain class objects
	# comparisons: list of domain pairs for comparisons, contains strings of the form
	#              'YAL017W;1090-1354:YCL024W;18-286'
	# map_comp_to_type: dictionary; keys are strings of the form 'YAL017W;1090-1354:YCL024W;18-286'
	#                               values are 'related' or 'not' depending on the type of comparisons to be run

	# returns: nothing, but writes a *_pairwise.txt output file to out_path
	ofile = open(out_path, 'w')
	c1 = 0
	c2 = 0
	for comp in comparisons:
		# determine whether this comparison is between related or unrelated domains
		if map_comp_to_type[comp] == 'related':
			c1 += 1
			c2 = 0
			# if related,
			# compare the two aligned profiles using the slowMidFast score
			result = slowMidFast(alignedDomains[comp][0], alignedDomains[comp][1])
			ofile.write(str(c1)+'\t'+str(c2)+'\trelated\t'+comp+'\t'+'%.9f' %result+'\n')
		else:
			c2 += 1
			# if unrelated,
			# align the single-domain profiles for the two domains
			d1_profile, d2_profile = domains[comp.split(':')[0]].singleDomainProfile, domains[comp.split(':')[1]].singleDomainProfile
			d1_aligned, d2_aligned = alignSingleDomainProfiles_minmax(d1_profile, d2_profile)
			result = slowMidFast(normProfile(d1_aligned[:,1]), normProfile(d2_aligned[:,1]))
			ofile.write(str(c1)+'\t'+str(c2)+'\tnot\t'+comp+'\t'+'%.9f' %result+'\n')
	return

def runRandCodonUsageComparisons(out_path, alignedDomains, domains, comparisons, comp_types):
	...
	# comparisons: list of domain pairs for comparisons, contains strings of the form
	#              'YAL017W;1090-1354:YCL024W;18-286'
	# comp_types: list of the type of comparison to perform; must exactly match the order of comparisons
